 ...
 */
export function getPrompt(name, args, ctx) {
 ...
 */
function getRelevantContext(promptName, ctx) {
    const parts = [];
    ...
    const relevantWorkflows = workflows.filter(w => keywords.some(k => w.name.toLowerCase().includes(k) ||
 ...
 * Build prompt messages
 */
function buildPromptMessages(promptName, args, context) {
    const systemPrompts = {
