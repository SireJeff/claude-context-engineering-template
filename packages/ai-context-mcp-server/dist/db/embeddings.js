...
export class EmbeddingsManager {
     ...
     */
    async storeEmbedding(contextId, content) {
        // Generate embedding
     ...
     */
    async storeEmbeddingsBatch(items) {
     ...
     */
    async search(query, limit = 10, minSimilarity = 0.5) {
      ...
      LIMIT ?
    `);
        const rows = stmt.all(queryBuffer, limit * 2);
     ...
     */
    async findSimilar(contextId, limit = 10) {
    ...
    `);
      ...
      LIMIT ?
    `);
        const rows = searchStmt.all(row.embedding, contextId, limit);
        return rows.map(row => ({
            contextId: row.context_id,
            distance: row.distance,
            similarity: 1 / (1 + Math.sqrt(row.distance))
    ...
    async processQueue(batchSize = 10) {
      ...
      LIMIT ?
    `);
        const rows = getStmt.all(batchSize);
        if (rows.length === 0) {
            return 0;
        }
        // Mark as processing
    ...
    `);
        ...
        // Generate embeddings
        try {
            await this.storeEmbeddingsBatch(rows.map(row => ({
                contextId: row.context_id,
                content: row.content
            })));
            // Mark as completed
