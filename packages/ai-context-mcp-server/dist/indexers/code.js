...
export class CodeIndexer {
    ...
    projectRoot;
    maxChunkSize;
    constructor(db, embeddings, projectRoot, maxChunkSize = 2000) {
        this.db = db;
     ...
     */
    async indexLanguage(language, config) {
        const result = {
            files: 0,
            chunks: 0,
            errors: []
     ...
     */
    chunkCode(content, filePath, language) {
        const chunks = [];
     ...
     */
    createCodeSummary(content, filePath, language, entityName) {
        const parts = [
            `File: ${filePath}`,
            `Language: ${language}`
        ];
